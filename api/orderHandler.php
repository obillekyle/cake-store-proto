...
if (isset($_POST["method"])) {
  ...
  if ($method == "orderConf") {
    ...
    for ($i = 0; $i < count($ids); $i++) {
      array_push($values, "('".join("', '", [$count[$i],0,0,$items[$i],$details[$i],$prices[$i],$u_id,"Name: ".$info->name." \nPhone Number: ".$info->phone." \nAddress: ".$info->address])."')");
    }

    $sql = "INSERT INTO orders (quantity,fulfilled,denied,item_id,details,value,user_id,info) VALUES ". join(",", $values);
  ...
  // Security Check
  if ($_SESSION['role'] != 'Admin') response(false, "Not Allowed");
  // Accept Orders
  if ($method == "accept") {
...
if (isset($_GET["orders"])) {
  ...
  while ($row = mysqli_fetch_assoc($res)) $a = [...$a, $row];
...
if ($_SESSION['role'] != 'Admin') response(false, "Not Allowed");
$sql = "SELECT o.id as o_id, i.id as i_id, i.name, u.username as user,o.details as 'desc',
               o.info, o.quantity as amnt, CONCAT('$currency', o.value) as cost
        FROM orders AS o 
        JOIN items as i ON o.item_id = i.id 
        JOIN users as u ON o.user_id = u.id
